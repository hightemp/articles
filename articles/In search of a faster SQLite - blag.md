...
![](https://avi.im/blag/images/2024/penberg-paper-cover.jpg)
...
There are two things I really loved in this paper: their explanation of query execution and how io\_uring works.
...
> 
> With io\_uring, the application first calls the `io_uring_setup()` system call to set up two memory regions: the submission and completion queues. The applications then submit I/O requests to the submission queue and call the `io_uring_enter()` system call to tell the OS to start processing the I/O requests. However, unlike the blocking `read()` and `write()`calls, `io_uring_enter()` does not block the thread by returning control to the userspace. The application can now perform other work concurrently and periodically poll the completion queue in userspace for I/O completion.
...
Why can’t we just move SQLite to io\_uring?
...
In other words, you need to rewrite much of SQLite. The researchers chose a different approach: rewrite SQLite in Rust using io\_uring.

### Limbo

![](https://avi.im/blag/images/2024/limbo-arch.png)
...
![](https://avi.im/blag/images/2024/op-codes.png)
...
![](https://avi.im/blag/images/2024/byte-code.png)
...
![](https://avi.im/blag/images/2024/penberg-results.png)
...
The Limbo code is open source: [https://github.com/tursodatabase/limbo](https://github.com/tursodatabase/limbo)
